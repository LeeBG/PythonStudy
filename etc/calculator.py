...
def validate(menu):  # 메뉴 처리 및 계산식을 검증을 하는 함수()
  ...
  num1 = ""          # 계산식의 앞의 숫자를 저장할 변수(나중에 int로 형변환 할거임)
  num2 = ""          # 계산식의 뒤의 숫자를 저장할 변수(나중에 int로 형변환 할거임)
  operator = ""      # 연산자
...
def calculator(value):          # 계산 및 기록을 하는 함수()
  ...
  if operator == '/':
    if value[2] != 0:
      ...
      history.append("%d %s %d = %f"%(value[0],value[1],value[2],result))   # float형은 소수점 아래 모두 기록
      return record
    else:
      print("이해할 수 없는 연산입니다")
      return
    ...
    result = value[0]*value[2]
    record = "%d %s %d = %d"%(value[0],value[1],value[2],result)
  history.append(record)                                          # 올바른 연산 만 기록하기
  return record
...
def start_calulator() :          # 전체 로직을 총괄하는 함수
  while True:                    # 종료메뉴 선택시 까지 무한반복
    ...
    if value != None:            # 검증된 유효한 값이라면..
      result = calculator(value) # 계산 및 기록
    if result != None:           # 예외 발생 시 출력 X
      print(result)              # 결과값 출력
    os.system("pause")
    os.system("cls")
